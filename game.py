...
def main():
    ...
    macGyver = MacGiver(startX, startY, speed)
    ...
    while run:
        ...
        if keys[pygame.K_LEFT]\
                and macGyver.x > macGyver.speed:
            macGyver.moveRight(limit)
        if keys[pygame.K_RIGHT]\
                and macGyver.x < winWidth - macGyver.speed - macGyver.width:
            macGyver.moveLeft(limit)
        if keys[pygame.K_UP]\
                and macGyver.y > macGyver.speed:
            macGyver.moveUp(limit)
        if keys[pygame.K_DOWN]\
                and macGyver.y < winHeigth - macGyver.speed - macGyver.heigth:
            macGyver.moveDown(limit)

        # Item management
        if (macGyver.x, macGyver.y) == items.needlePosition\
                and items.pop['needle']:
            items.pop['needle'] = False
            count += 1
        if (macGyver.x, macGyver.y) == items.tubePosition\
                and items.pop['tube']:
            items.pop['tube'] = False
            count += 1
        if (macGyver.x, macGyver.y) == items.etherPosition\
                and items.pop['ether']:
